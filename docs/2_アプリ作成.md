...
> [!NOTE]
>
> 実行するリージョンごとにBootstrapを実行する必要があります。
>
> バージニア北部リージョンにてCDKを利用したことがない場合は、他のリージョンで作業した実績があったとしても、本作業を実施する必要があります。
...
>
> Kiroによる編集に失敗した場合は、`workshop/create-login` ブランチを利用してください。
...
`amplify/` 以下のファイル変更を検知して、 sandboxサーバが自動的にデプロイ開始してくれます。
...
> [!NOTE]
>
> Todoアプリを追加したブランチを用意しています。
>
> Kiroによる編集に失敗した場合は、`workshop/create-todo-app` ブランチを利用してください。
...
>
> Kiroによる編集に失敗した場合は、`workshop/created` ブランチを利用してください。
